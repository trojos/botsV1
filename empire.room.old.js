...
var empireroom = {
    run: function (room, subrooms) {
        ...
        if (Memory.rooms[room].Bmstr.constructionsites != bmstrsites.length || Memory.rooms[room].Bmstr.tick < Game.time - 15) {
            ...
            var gesamthits = _.sum(repramp, 'hits') + _.sum(buildwall, 'hits')
            var averagehits = Math.floor(gesamthits / (repramp.length + buildwall.length))
            var minhitswall = _.min(buildwall, 'hits')
            var minhitsramp = _.min(repramp, 'hits')
            //console.log(JSON.stringify(minhitswall))
            var minhits = 0
            if (minhitswall.hits < minhitsramp.hits) { minhits = minhitswall } else { minhits = minhitsramp }
            Memory.rooms[room].Bmstr.Defence = {}
            Memory.rooms[room].Bmstr.Defence.averagehits = averagehits
            Memory.rooms[room].Bmstr.Defence.minid = minhits.id
            Memory.rooms[room].Bmstr.Defence.minhits = minhits.hits
            Memory.rooms[room].Bmstr.Defence.minpos = minhits.pos

            Memory.stats['rooms.' + room + '.defence.averagehits'] = averagehits
            Memory.stats['rooms.' + room + '.defence.minhits'] = minhits.hits
        ...
        if (Memory.rooms[room].links == undefined || Memory.rooms[room].links.tick == undefined || Memory.rooms[room].links.tick < Game.time - 501) {
            ...
            var alllinks = Game.rooms[room].find(FIND_MY_STRUCTURES, {
                filter: st => st.structureType == STRUCTURE_LINK && st.isActive()
            })
            if (alllinks.length > 0) {
                ...
                // upgradelink
                var upgradelinks = Game.rooms[room].controller.pos.findInRange(FIND_MY_STRUCTURES, 4, {
                    filter: st => st.structureType == STRUCTURE_LINK && st.isActive()
                })
                i = 0
        ...
        allcreeps.forEach(creep => {
            if (creep.memory.role == 'carry' || creep.memory.role == 'carry2') {
                ...
                upgradercreeps += 1
                upgradercreepsBP += creep.getActiveBodyparts(WORK)
        ...
        //Miner je Spot einer! Definition unten bei spawnen
        //upgrader (bis havestore)
        if (!havestore) {
            if (HomeRCL < 4) { upgrader = 3 * HomeRCL } else { upgrader = 10 } //Max Anzahl an Creeps
        ...
        //carry
        if (HomeRCL < 4) {
            carrycreepBPneed = HomeRCL * 30
        } else {
            var carrycreepBPneed = 20
            if (xspawns.length > 0) {
                carrycreepBPneed += xspawns.length * 30
            }
            if (Memory.rooms[room].links != undefined && Memory.rooms[room].links.transfer != undefined) {
                carrycreepBPneed -= Memory.rooms[room].links.transfer.length * 10
            }
        }

        //console.log(room, Memory.rooms[room].creeps.carry.BP - carrycreepBPneed)
        ...
        var notfall = false
        if (HomeRCL > 1 && (carrycreeps < 1 || minercreeps < 1)) {
        ...
        if (Game.time % 6 == 0) {                 // Nur jeden 6. Tick ausführen damit wird viel CPU gespart!!!!
            //---------- Scout -----------------
            ...
            if (havestore) {
                    ...
                    if (!needminer) {  //zuerst werden miner gebaut, wenn vorhanden dann carry
                        if (linkminer) {            // Wenn Links vorhanden gibt es keine Carry von Spot zu Lager
                            ...
                            if (HomeRCL < 7) {
                                ...
                                if (invtype == 'big') {                 // Wenn Invasion dann mehr Carry damit die Türme gefüllt werden
                                    var carryneed = 18 + (spawns.length - 1) + 10 + 30
                                } else {
                                    var carryneed = 18 + (spawns.length - 1) + 10
                                }
                            }
                        } else {
                        ...
                        if (bodyparts < carryneed) {
                            ...
                            if (Game.rooms[room].memory.notfall) { if (haveenergy > 2250) { creepsize = 2250 } else { creepsize = haveenergy }; console.log('Notfall') }
                ...
                //spawn miner mit spot im memory
                var cmem = { role: 'miner', spot: miningspots, home: room, targetroom: room }
                var cbody = [WORK, WORK, MOVE]
                if (maxcreepsize > 800) { creepsize = 800 } else { creepsize = maxenergy }    // Beschränkt maxcreepsize
                if (Game.rooms[room].memory.notfall) { creepsize = haveenergy; console.log('Notfall') }       // Falls Notfall!
                var ccreep = _.filter(Game.creeps, (creep) =>
            ...
            //---------- CARRY  ----------
            if (1 == 2) {            // Wenn Links vorhanden gibt es keine Carry von Spot zu Lager
                // Sondern ab centerlink zur befüllung/Lager
                var cmem = { role: 'carry2', home: room, targetroom: room, harvesting: true }
                if (Game.rooms[room].memory.notfall) { creepsize = haveenergy; console.log('Notfall') }       // Falls Notfall!

                if (Memory.rooms[room].creeps.carry.BP < carryneed && !needminer) {
                    needcarry = true
                    var cbody = [CARRY, CARRY, MOVE]
                    var cbodycost = 75
                    var bodypartsneed = carryneed - bodyparts
            ...
            if (!needminer && !needcarry && !needbmstr && !needdef) {
                if (havestore) {
                    var upgraderstore = 0
                    if (reserve >= 50000) {
                        upgraderstore = Math.ceil((reserve - 50000) / 50000)
                        var minimal = false
                    } else {
                        var minimal = true
                    }
                    ...
                    if (subattack || minimal) {
                        ...
                        if (maxcreepsize > 2400) { creepsize = 2400 } else { creepsize = maxcreepsize }    // Beschränkt maxcreepsize
                    } else {
                        if (maxcreepsize > 3000) { creepsize = 3000 } else { creepsize = maxcreepsize }    // Beschränkt maxcreepsize
                    }
                    Memory.rooms[room].upgrader.BPneed = bpn
                    ...
                    if (otherspots.length > 0) {
                        for (var i in otherspots) {
                            ...
                            //console.log(bpn)
                            Memory.rooms[room].upgrader.BPneed = bpn
                            var cmem = { role: 'upgrader', spot: miningspots.id, home: room, targetroom: room, upgrading: false }
        ...
        if (DDneed > 0) {
            var cmem = { role: 'FernDD', home: room, targetroom: room, status: 'attack' }
            if (HomeRCL < 7) {
            ...
            if (ccreep.length < DDneed) {
                creepspawn.newcreep(room, 'defenderFDD', creepsize, cbody, cmem)
                needdef = true
            }
        }
        if (DDneed > 1) {
            var cmem = { role: 'Heiler', home: room, targetroom: room, status: 'attack' }
            if (HomeRCL < 7) {
            ...
            //&& creep.memory.status == cmem.status);
            if (ccreep.length < 1) {
                creepspawn.newcreep(room, 'defenderHeiler', creepsize, cbody, cmem)
                needdef = true
            }
        }
        //Attack
        var defender = _.filter(Game.creeps, (creep) =>
            (creep.memory.role == 'FernDD' || creep.memory.role == 'Heiler')
        ...
        if (tellenergy) {
            ...
            structures.forEach(struc => {
                if (struc.structureType == STRUCTURE_SPAWN && struc.energyCapacity > struc.energy && struc.isActive()) {
                ...
                if (struc.structureType == STRUCTURE_TOWER && struc.energyCapacity > struc.energy + 100 && struc.isActive()) {
                ...
                if (!invasion) {
                    if (struc.structureType == STRUCTURE_LAB && struc.energyCapacity > struc.energy + 500 && struc.isActive()) {
            ...
            if (energyneed.length < 20 || invasion) {       //Wenn viele Objekte Energie brauchen wird Terminal nicht beliefert damit die anderen schneller geladen werden
                var term = Game.rooms[room].terminal
                if (term != undefined) {
                    if (term.store.energy < 21000 && term.isActive()) {
                        energyneed.push({ id: term.id, type: STRUCTURE_TERMINAL, res: RESOURCE_ENERGY, amount: 22000 - term.store.energy, pos: term.pos })
                    }
                    //console.log(_.sum(term.store))
                }
            }
            ...
            if (reserve > 180000 && energyneed.length < 10 && !invasion) {
                //if (Game.time % 10 == 0) {
                var nuker = Game.rooms[room].find(FIND_MY_STRUCTURES, { filter: str => str.structureType == STRUCTURE_NUKER })
                if (nuker.length > 0) {
                    energyneed.push({ id: nuker[0].id, type: STRUCTURE_NUKER, res: RESOURCE_ENERGY, amount: nuker[0].energyCapacity - nuker[0].energy, pos: nuker[0].pos })
                }
                //}
            }
            ...
            if (energyneed.length > 20 && havestore && reserve > 100000) {  //Wenn viele Objekte Enegrie brauchen wird nur vom Store genommen damits schneller geht
                var priostore = true
            } else { var priostore = false }


            if (!priostore) {
                if (invasion) {    //Wenn Invasion dann werden dropps nur im Kernbereich aufgehoben damit die Carry nicht außer die Mauern gehen

                    var dropped = Game.rooms[room].storage.pos.findInRange(FIND_DROPPED_RESOURCES, 10)
                ...
                var term = Game.rooms[room].terminal
                if (term != undefined) {
                    if (term.store.energy > 25000 && term.isActive()) {
                                ...
                                Aufträge.forEach(Auftrag => {
                                    if (Auftrag.Art == 'get') {
                                        if (Memory.rooms[room].energy.have[Auftrag.id] != undefined) {
                                            ...
                                            if (Auftrag.type == 'drop') {
                                                creep.memory.Aufträge = []      //Wenn der Auftrag in der Liste nicht mehr ist, dann wird dieser auch aus Creep memory geläscht
                                            }
                                        }
        ...
        if (HomeRCL >= 6) {
            ...
            //extractor bauen
            var mineralspot = Game.rooms[room].find(FIND_MINERALS)[0]
            ...
            if (extractstr.length > 0) { var haveextr = true } else {
                var haveextr = false
                Game.rooms[room].createConstructionSite(mineralspot.pos.x, mineralspot.pos.y, STRUCTURE_EXTRACTOR)
            }
        ...
        for (var i in subrooms) {
            ...
            if (subrooms[i].todo == 'attack') {
                subroomattack.run(room, subrooms[i].targetroom, wait, subrooms[i].type, subrooms[i].NahDD, subrooms[i].FernDD, subrooms[i].Heiler, subrooms[i].Dismantler, subrooms[i].attackcontroller, subrooms[i].boost)
                ...
                if (Memory.Empire.Attack[subrooms[i].targetroom] == undefined) {
                    ...
                    Memory.Empire.Attack[subrooms[i].targetroom].boost = 0
                    ...
                    Memory.Empire.Attack[subrooms[i].targetroom].Dismantler = 0
                    Memory.Empire.Attack[subrooms[i].targetroom].from = []
                }
                Memory.Empire.Attack[subrooms[i].targetroom].type = subrooms[i].type
                Memory.Empire.Attack[subrooms[i].targetroom].boost = subrooms[i].boost
                ...
                Memory.Empire.Attack[subrooms[i].targetroom].Dismantler += subrooms[i].Dismantler
        ...
        // Energy ausgleich
        if (Game.rooms[room].terminal && Game.rooms[room].storage) {  // Ohne Terminal und Storage geht kein Ausgleich
            if (Game.rooms[room].terminal.isActive()) {
                var energymin = 0.85  //Standart Prozentwerte basierend auf Durchschnitt aller
                var energymax = 1.05
                Memory.Empire.rooms[room].Energie = { store: stored, terminal: storedterminal, min: energymin, max: energymax }
            }
        }
        // Minerals in Memory schreiben
        if (Game.rooms[room].terminal) {
            if (Game.rooms[room].terminal.isActive()) {
                Memory.Empire.rooms[room].Minerals = Game.rooms[room].terminal.store
                //delete Memory.Empire.rooms[room].Minerals.energy
            }
        }
    }

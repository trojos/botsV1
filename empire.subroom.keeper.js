...
var subroom = {
    run: function (room, subroom, wait) {
        ...
        if (Game.rooms[room].storage == undefined) {
            var havestore = false
            var reserve = 0
            var Lager = { id: xspawns[0].id, pos: xspawns[0].pos }
        } else {
            var havestore = true
            var reserve = Game.rooms[room].storage.store[RESOURCE_ENERGY]
            var Lager = { id: Game.rooms[room].storage.id, pos: Game.rooms[room].storage.pos }
        }

        Memory.rooms[subroom].Lager = Lager
        ...
        if (insight) {
            var center = Game.rooms[subroom].isCenter()
        }
        // Rep und Bauzeugs in Memory schreiben
        if (insight) {

            if (Memory.rooms[subroom].plaincm == undefined) {
                var cmplain = new PathFinder.CostMatrix
                let terrains = Game.rooms[subroom].lookForAtArea(LOOK_TERRAIN, 0, 0, 49, 49, true);
                var cpucmvor = Game.cpu.getUsed()
                for (let i = 0; i < terrains.length; i++) {
                    let x = terrains[i].x;
                    let y = terrains[i].y;
                    let terrain = terrains[i].terrain;
                    if (terrain === 'swamp') {
                        cmplain.set(x, y, 10);
                        new RoomVisual(subroom).text('S', x, y, { opacity: 0.2 })
                    } else if (terrain === 'plain') {
                        cmplain.set(x, y, 2);
                        new RoomVisual(subroom).text('P', x, y, { opacity: 0.2 })
                    } else if (terrain === 'wall') {
                        cmplain.set(x, y, 0xff);
                        new RoomVisual(subroom).text('W', x, y, { opacity: 0.2 })
                    }
                }
                Memory.rooms[subroom].plaincm = JSON.stringify(cmplain.serialize())
            }
            ...
            //Spots in Memory schreiben

            if (Memory.rooms[subroom] == undefined) { Memory.rooms[subroom] = {} }
            if (Memory.rooms[subroom].spots == undefined) { Memory.rooms[subroom].spots = {} }
            if ((Memory.rooms[subroom].spots.tick == undefined || Memory.rooms[subroom].spots.tick < Game.time - 500) || subroom == 'W6S14..') {
                Memory.rooms[subroom].spots = {}
                Memory.rooms[subroom].spots.tick = Game.time
                Memory.rooms[subroom].spots.spots = {}
                if (insight) {
                    ...
                    sources.forEach(function (spot) {
                        Memory.rooms[subroom].spots.spots[spot.id] = {}
                        var path = PathFinder.search(spot.pos, { pos: xspawn.pos, range: 1 }, {
                        ...
                        Memory.rooms[subroom].spots.spots[spot.id].carrybpneed = Math.ceil(carrybpneed)
                        if (spot.energyCapacity == undefined) {
                            Memory.rooms[subroom].spots.spots[spot.id].mineral = true
                        } else {
                            Memory.rooms[subroom].spots.spots[spot.id].mineral = false
                        }
                        Memory.rooms[subroom].spots.spots[spot.id].id = spot.id
                        Memory.rooms[subroom].spots.spots[spot.id].cost = pathcost
                        var lairbyspot = spot.pos.findInRange(FIND_HOSTILE_STRUCTURES, 5, {
                            filter: { structureType: STRUCTURE_KEEPER_LAIR }
                        })
                        //Berechne Waitpoint für FernDD
                        if (lairbyspot.length > 0 || spot.length > 0) {
                            Memory.rooms[subroom].spots.spots[spot.id].lair = lairbyspot[0].id
                            console.log('lair', lairbyspot[0].pos)
                            console.log('spot', spot.pos)
                            ...
                            console.log('waitpoint', JSON.stringify(waitpoint))
                            new RoomVisual(subroom).circle(waitpoint.x, waitpoint.y, { fill: '#00ba00', radius: .5 })
                            Memory.rooms[subroom].spots.spots[spot.id].waitpoint = waitpoint


                        }
                        //Berechne Fleepoint für Arbeiter wenn Lair kurz vor Spawn oder Keeper aktiv für FernDD
                        if (lairbyspot.length > 0 || spot.length > 0) {
                            if (lairbyspot[0] == undefined) { lairbyspot[0] = spot[0] }
                            if (spot == undefined) { spot = lairbyspot[0] }
                            var goals = []
                            goals.push({ pos: lairbyspot[0].pos, range: 6 })
                            goals.push({ pos: spot.pos, range: 6 })
                            var fleepath = PathFinder.search(spot.pos, goals, {
                                flee: true, plainCost: 2, swampCost: 10,
                                roomCallback: function (roomName) {
                                    let room = Game.rooms[roomName];
                            ...
                            var fleepoint = fleepath.path[fleepath.path.length - 1]
                            new RoomVisual(subroom).circle(fleepoint.x, fleepoint.y, { fill: '#ff0000', radius: .5 })
                            Memory.rooms[subroom].spots.spots[spot.id].fleepoint = fleepoint
                            Memory.rooms[subroom].spots.spots[spot.id].fleepoint.cost = fleepath.cost
                            buildroad.run(spot.pos, fleepoint, 1, 1)
                        }
                    ...
                    if (HomeRCL < 7) {
                        if (sources.length > 2) {
                            for (var i = 2; i < sources.length; i++) { // i = Anzahl der spots die bearbeitet werden sollen
                                spotcosts = Memory.rooms[subroom].spots.spots
                                var maxcost = _.max(spotcosts, 'cost')
                                delete Memory.rooms[subroom].spots.spots[maxcost.id]
                            }
                        }
                    }
                }
        ...
        if ((Memory.rooms[subroom].Straßenbau.tick == undefined) || Memory.rooms[subroom].Straßenbau.tick < Game.time - 1000 && Memory.rooms[subroom].Straßenbau.fertig == true) {
            ...
            if (Memory.rooms[subroom].keeper.tick == undefined || Memory.rooms[subroom].keeper.tick < Game.time - 0) {
                ...
                if (invaders.length > 0) {
                    ...
                    if (center) {
                        ...
                        if (inBPheal >= 20) { invtype = 'big'; DDneed = 3 }
                        if (inBPheal > 45) { invtype = 'verybig'; DDneed = 4 }
                ...
                var cpucmvor = Game.cpu.getUsed()
                var cmsave = PathFinder.CostMatrix.deserialize(JSON.parse(Memory.rooms[subroom].plaincm))

                Game.rooms[subroom].find(FIND_STRUCTURES).forEach(function (struct) {
                    if (struct.structureType === STRUCTURE_ROAD) {
                        cmsave.set(struct.pos.x, struct.pos.y, 1);
                    } else if (struct.structureType === STRUCTURE_KEEPER_LAIR) {
                        if (struct.ticksToSpawn < 10) {
                            var dx = 3
                            var dy = 3
                            for (var x = 0; x < (2 * dx) + 1; x++) {
                                var ex = struct.pos.x - dx + x
                                for (var y = 0; y < (2 * dy) + 1; y++) {
                                    var ey = struct.pos.y - dy + y
                ...
                for (var i in keepers) {
                    ...
                    var dy = 3
                    for (var x = 0; x < (2 * dx) + 1; x++) {
                        var ex = keepers[i].pos.x - dx + x
                        for (var y = 0; y < (2 * dy) + 1; y++) {
                            var ey = keepers[i].pos.y - dy + y
                            cmsave.set(ex, ey, 255)
                        }
                    }
                }
                if (invaders.length > 0) {
                    for (var i in invaders) {
                        ...
                        var dy = 3
                        for (var x = 0; x < (2 * dx) + 1; x++) {
                            var ex = invaders[i].pos.x - dx + x
                            for (var y = 0; y < (2 * dy) + 1; y++) {
                                var ey = invaders[i].pos.y - dy + y
                                cmsave.set(ex, ey, 255)
                            }
                        }
                    }
        ...
        if (Game.time % 3 == 0) {
            ...
            if (maxenergy >= 5600 && !center) {
                ...
                if (maxenergy >= 6300) {
                    var cbody = [MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, HEAL, HEAL, HEAL, HEAL, HEAL]
                } else {
                ...
                } else if (ccreep2.length > NahDD && !invasion) {
                    dcreep = _.min(ccreep2, 'ticksToLive')
                    if (dcreep != undefined) {
            ...
            if (insight && !needheal && !needFernDD && !needsourcedef && !invasion) {
                var miningspots = Memory.rooms[subroom].spots.spots
                for (var spots in miningspots) {
                    if (!miningspots[spots].mineral) {
                        ...
                        if (!needminer && !needsourcedef && !wait && !constructing) {
                            ...
                            if (Memory.rooms[subroom].spots.spots[miningspots[spots].id].carrybpneed == undefined) {
                                Memory.rooms[subroom].spots.tick = 0
                                Memory.rooms[subroom].spots.spots[miningspots[spots].id].carrybpneed = 10
                            }
                            Memory.rooms[subroom].spots.spots[miningspots[spots].id].carrybphave = bodyparts
                            var BPneed = Memory.rooms[subroom].spots.spots[miningspots[spots].id].carrybpneed
                            //var BPneed = 20
                            if (bodyparts < BPneed) {
                                needcarry = true

...
var squadattack = {
    run: function (targetroom) {
        ...
        for (var name in Game.creeps) {                //Creeps ohne Status wird wait zugewiesen
            var creep = Game.creeps[name];
            if (creep.memory.role == 'NahDD' && creep.memory.targetroom == targetroom) {
                if (creep.memory.status == undefined || creep.memory.status == 'wait') {
        ...
        if (arAlle.length == 0) {
            ...
            if (insight) {                                  //<---------- Anzahl der Creeps die für einen Angriff erforderlich sind
                ...
                if (hostile.length < 1) {
                    ...
                    //sammeln:
                    var mcNahDD = 2; var mcFernDD = 0; var mcHeiler = 3; //<--- Normaler Angriff
                    //im Kampf:
                    var fcNahDD = 5; var fcFernDD = 0; var fcHeiler = 6;
                }
            } else {
                //sammeln:
                var mcNahDD = 3; var mcFernDD = 0; var mcHeiler = 3;
                //im Kampf:
                var fcNahDD = 5; var fcFernDD = 0; var fcHeiler = 5;
            }
            azFernDD = 0

...
var subroom = {
    run: function (room, subroom, wait) {
        ...
        if (insight) {
            ...
            if (Memory.rooms[subroom].Wege.tick == undefined || Memory.rooms[subroom].Wege.tick < Game.time - 155) {
                ...
                if (insight) {
                    ...
                    sources.forEach(function (spot) {
                        ...
                        if (lairbyspot.length > 0 || spot.length > 0) {
                            ...
                            var goals = []
                            goals.push({ pos: lairbyspot[0].pos, range: 4 })
        ...
        if (!invasion) {
            ...
            //spawnt Source Def
            // if (maxenergy > 4000) {
            //     if (invasion || !insight) { var NahDD = 2 } else { var NahDD = 1 }
            //     var cmem = { role: 'keeper_NahDD', home: room, targetroom: subroom }
            //     var cbody = [TOUGH, TOUGH, TOUGH, TOUGH, TOUGH, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, MOVE, MOVE, HEAL, HEAL, HEAL, HEAL]
            ...
            //         needsourcedef = true
            //         creepcost = creepspawn.newcreep(room, 'k_NahDD_' + subroom, creepsize, cbody, cmem)
            ...
            if (insight) {
                ...
                //----------  Miner + Carry  ----------
                //Je Miner 2 Carrier
                if (insight && !needheal && !needFernDD && !needsourcedef) {
                    ...
                    for (var spots in miningspots) {
                        if (!miningspots[spots].mineral) {
                            ...
                            if (!needminer && !needsourcedef && !wait) {
                            ...
                            if (!needsourcedef && !wait && !mempty && haveterm) {
                ...
                //---------- bmstr Baumeister  ----------
                if (insight && !wait && !needheal && !needFernDD && !needsourcedef && !needminer) {
        ...
        if (needminer || needcarry || needbmstr || needdef || needheal || needFernDD || needsourcedef) {
            var wait = true
        } else { var wait = false }

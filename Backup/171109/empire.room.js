...
var empireroom = {
    run: function (room, subrooms) {
        ...
        if (Memory.rooms[room].Bmstr.tick == undefined || Memory.rooms[room].Bmstr.tick < Game.time - 1) {
            Memory.Bmstr = {}
                ...
                for (var spots in miningspots) {
                    ...
                    if (!needminer) {  //zuerst werden miner gebaut, wenn vorhanden dann carry
                        if (linkminer) {            // Wenn Links vorhanden gibt es keine Carry von Spot zu Lager
                            ...
                            var cmem = { role: 'carry', spot: centerlinkid, home: room, targetroom: room, harvesting: true }
                            var carryneed = 1
                            if (maxcreepsize > 2400) { creepsize = 2400 } else { creepsize = maxcreepsize }
                        } else {
                            var cmem = { role: 'carry', spot: miningspots[spots].id, home: room, targetroom: room, harvesting: true }
                            var carryneed = 2
                            if (maxcreepsize > 1400) { creepsize = 1400 } else { creepsize = maxcreepsize }
                        }
        ...
        if (!needminer && !needcarry && !needbmstr && !needdef) {
            if (havestore) {
                var upgraderstore = 0
                if (reserve >= 20000) {
                    upgraderstore = Math.ceil((reserve - 20000) / 50000)
                    var minimal = false
                } else {
                    var minimal = true
                }
                ...
                if (otherspots.length > 0) {
                    for (var i in otherspots) {
                        ...
                        var ept = 50 / (25 + 50 + (pathlength * 2) + 2) * 300  //energy pro bodypart pro 300ticks
        ...
        for (var i in subrooms) {
            if (subrooms[i].todo == 'harvest') {
                wait = subroom.run(room, subrooms[i].targetroom, wait)
            }
            ...
            if (subrooms[i].todo == 'mineral') {
                mineral.run(room, subrooms[i].targetroom, wait)
            }

...
var subroomattack = {
    run: function (room, subroom, wait) {
        ...
        var creepsize
        //Game.creeps[]
//console.log(Game.creeps[ca_E97S81_3868])
        //----------Scout-----------
        if (!insight) {
        ...
        //----------Nahkampf-----------
        if (true) {
            var nahdd = 2           //Regelanzahl  (teils angriff, teils warten, daher ca. das doppelte das gleichzeitig angreifen soll)
        ...
        if (true) {
            ...
            if (heiler > 0) {
                ...
                maxcreepsize = haveenergy
                if (maxcreepsize > 4300) { creepsize = 4300 } else { creepsize = maxcreepsize }    // Beschränkt maxcreepsize
                var ccreep = _.filter(Game.creeps, (creep) =>
                    creep.memory.role == cmem.role
                    && creep.memory.home == cmem.home
                    && creep.memory.targetroom == cmem.targetroom);

...
module.exports.loop = function () {
    ...
    for (const room in AllMineralRooms) {
        ...
        for (const mineral in Ausgangsmaterial) {
            ...
            if (Memory.rooms[room].Labs == undefined) {
                ...
                if (Memory.rooms[room].Labs.Lager == undefined) {           // Für Räume kleiner RCL 8 wird empire.reactions nicht ausgeführt, daher ist Lager nicht definiert
                    var Lagerneed = 0
                    var Lagerhave = 0
                } else {
    ...
    Memory.stats['CPU.role.deliver'] = 0
    ...
    Memory.stats['creeps.role.deliver'] = 0
    ...
    // var mtype = 'XGH2O'
    // console.log(mtype)
    // for (const sroom in srooms) {
    //     if (srooms[sroom].Minerals) {
    //         if (srooms[sroom].Minerals[mtype] > 0) {
    //             var interm = srooms[sroom].Minerals[mtype]
    //             var intermsum = _.sum(Game.rooms[sroom].terminal.store)
    //         }
    //     }
    //     slabs = Game.rooms[sroom].find(FIND_STRUCTURES, { filter: str => str.structureType == STRUCTURE_LAB })
    //     var inlab = 0
    //     for (const slab in slabs) {
    //         //console.log(sroom, srooms[sroom].Minerals[mtype])
    //         if (slabs[slab].mineralType == mtype && slabs[slab].mineralAmount > 0) {
    //             inlab += slabs[slab].mineralAmount 
    //         }
    //     }
    //     if (interm > 0 || inlab > 0) {
    //         console.log(sroom,intermsum, 'in term',interm,'in lab',inlab )
    //     }
    // }
    ...
    Memory.succesfulltick = Game.time
}
